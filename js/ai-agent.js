...
function relevantKnowledge(knowledge,lead,company,service,latestMessage=''){
 const msg=norm(latestMessage), industry=norm(company?.industry), name=norm(company?.name);
 const terms=[...msg.split(/\W+/),...industry.split(/\W+/),...name.split(/\W+/)].filter(x=>x.length>2);
 const priority={pricing:5,services:5,faq:5,objections:4,sales:3,policies:3,general:1};
 return [...knowledge].map(k=>{
  const text=norm(`${k.title} ${k.content}`);
  let score=priority[category(k)]||1;
  if(service&&text.includes(norm(service)))score+=8;
  for(const t of terms)if(text.includes(t))score+=1;
  if(msg&&['faq','objections','sales'].includes(category(k)))score+=2;
  return{...k,_score:score};
 }).sort((a,b)=>b._score-a._score).slice(0,8);
}
function extractPrice(knowledge,service){
 const exact=knowledge.find(k=>category(k)==='pricing'&&norm(k.content).includes(norm(service)));
 return exact?.content||PRICES[service]||'';
}
...
function infer(lead,company,conversation,knowledge=[]){
 ...
 if(conversation?.status==='human_required'){score-=20;reasons.push('Human handoff is active.');}
...
export async function evaluateLead(leadId,{createLog=true}={}){
 ...
 const [cr,knowledge]=await Promise.all([
  db().from('conversations').select('id,status').eq('lead_id',leadId).order('updated_at',{ascending:false}).limit(1).maybeSingle(),loadKnowledge()
 ]);
 ...
 const d=await db().rpc('create_ai_decision',{p_lead_id:leadId,p_action:x.action,p_confidence:confidence,p_reasoning:reasoning,p_payload:{score:x.score,recommended_service:x.recommended_service,engine:'deterministic-safe-v5',conversation_id:cr.data?.id||null,knowledge_refs:knowledgeRefs}});
...
async function loadDrafts(conversationId=null){let q=db().from('ai_response_drafts').select('id,workflow_execution_id,action_id,lead_id,conversation_id,user_id,subject,body,status,knowledge_refs,created_at,updated_at,leads(name,companies(name)),conversations(subject,channel)').order('created_at',{ascending:false}).limit(100);if(conversationId)q=q.eq('conversation_id',conversationId);const r=await q;if(r.error)throw r.error;return r.data||[]}
...
async function queueDraftAsMessage(draft){if(!draft.conversation_id)throw new Error('This draft has no conversation.');if(draft.status!=='approved')throw new Error('Approve the draft before queueing it as a CRM message.');const c=await db().from('conversations').select('channel,lead_id').eq('id',draft.conversation_id).single();if(c.error)throw c.error;const r=await db().rpc('create_conversation_message',{p_conversation_id:draft.conversation_id,p_message:draft.body,p_sender_type:'human',p_channel:c.data.channel,p_ai_generated:false});if(r.error)throw r.error;const u=await updateDraft(draft.id,'queued');await recordActivity({type:'ai_response_draft_queued',title:'Approved AI response queued as CRM message',lead_id:draft.lead_id||c.data.lead_id,metadata:{draft_id:draft.id,conversation_id:draft.conversation_id}});return u}
...
async function renderDrafts(host,conversationId=null){host.innerHTML=loading('Loading response drafts…');try{const rows=await loadDrafts(conversationId);host.innerHTML=rows.map(d=>draftCard(d)).join('')||emptyState('No response drafts','Workflow-generated or manually prepared AI drafts will appear here.');qsa('[data-draft-action]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{const d=rows.find(x=>x.id===b.dataset.draftId);if(!d)throw new Error('Draft not found.');if(b.dataset.draftAction==='approve')await updateDraft(d.id,'approved');else if(b.dataset.draftAction==='reject')await updateDraft(d.id,'rejected');else await queueDraftAsMessage(d);toast(b.dataset.draftAction==='queue'?'Message queued in CRM':'Draft updated','success');await renderDrafts(host,conversationId)}catch(e){toast(e.message,'error');b.disabled=false}})}catch(e){host.innerHTML=emptyState('Draft center unavailable',e.message)}}

export async function openResponseDraft(conversationId){
 const c=await db().from('conversations').select('id,lead_id,channel,subject,leads(id,name,status,priority,recommended_service,company_id,companies(id,name,industry,city,website,email,phone,instagram,facebook,linkedin,google_maps_url))').eq('id',conversationId).single();
 if(c.error)throw c.error;
 const u=await getCurrentUser();if(!u)throw new Error('Authentication required.');
 const [knowledge,messages]=await Promise.all([loadKnowledge(30),db().from('messages').select('sender_type,message,created_at').eq('conversation_id',conversationId).order('created_at',{ascending:false}).limit(10)]);
 ...
 const focused=relevantKnowledge(knowledge,lead,company,rec?.service,latest?.message||'');
 const pricing=rec?extractPrice(focused,rec.service):'';
 const faq=focused.filter(k=>category(k)==='faq').slice(0,2);
 const objections=focused.filter(k=>category(k)==='objections').slice(0,2);
 const sales=focused.filter(k=>category(k)==='sales').slice(0,2);
 const serviceText=rec?`Based on ${company?.name||'your business'}'s current digital profile, ${rec.service} is the option we recommend.`:'';
 const priceText=pricing?`The current reference price is ${pricing}.`:'';
 const faqText=faq.length?`Relevant FAQ guidance: ${faq.map(k=>k.content).join(' ')}`:'';
 const objectionText=objections.length?`We can address the concern directly using this approved guidance: ${objections.map(k=>k.content).join(' ')}`:'';
 const salesText=sales.length?`Sales guidance: ${sales.map(k=>k.content).join(' ')}`:'';
 const defaultBody=`Hello ${lead?.name||'there'},\n\nThank you for your message. ${latest?'We reviewed your latest message and want to make the next step simple.':'We would be happy to discuss the best option for your business.'}\n\n${serviceText}\n${priceText}\n\n${faqText}\n\n${objectionText}\n\n${salesText}\n\nIf you would like, we can confirm the details and next step with you.\n\nBest regards,\nAI Agency` .replace(/\n{3,}/g,'\n\n').trim();
 const refs=focused.map(k=>({id:k.id,title:k.title,category:k.category}));
 const m=modal({title:'AI Response Draft',body:`<form class="form-grid" id="response-draft-form"><label class="full">Subject<input name="subject" maxlength="200" value="${safe(c.data.subject||`Follow-up with ${lead?.name||'lead'}`)}"></label><label class="full">Draft message<textarea name="body" rows="12" required maxlength="10000">${safe(defaultBody)}</textarea></label><div class="card full"><span class="eyebrow">AI CONTEXT</span><p class="muted">${focused.length} relevant knowledge item(s) selected from ${knowledge.length} active item(s).</p><div class="mini-list">${focused.map(k=>`<div class="list-row"><span><b>${safe(k.title)}</b><small>${safe(k.category)}</small></span></div>`).join('')||'<p class="muted">No matching knowledge yet. Add Services, Pricing, FAQ or Objections entries to improve drafts.</p>'}</div></div><div class="form-actions"><button type="button" class="btn btn-ghost" data-close>Cancel</button><button class="btn btn-primary">Save draft</button></div></form>`});
 m.querySelector('form').onsubmit=async e=>{e.preventDefault();const b=e.currentTarget.querySelector('.btn-primary');b.disabled=true;try{const fd=new FormData(e.currentTarget),body=String(fd.get('body')||'').trim();if(!body)throw new Error('Draft body is required.');const r=await db().from('ai_response_drafts').insert({lead_id:c.data.lead_id,conversation_id:conversationId,user_id:u.id,subject:String(fd.get('subject')||'').trim()||null,body,status:'pending',knowledge_refs:refs}).select().single();if(r.error)throw r.error;await recordActivity({type:'ai_response_draft_created',title:'Contextual AI response draft created',lead_id:c.data.lead_id,company_id:lead?.company_id,metadata:{draft_id:r.data.id,conversation_id:conversationId,knowledge_count:focused.length,recommended_service:rec?.service||null}});toast('AI draft saved','success');m.remove();const list=document.querySelector('#conversation-draft-list');if(list)renderDrafts(list,conversationId)}catch(x){toast(x.message,'error');b.disabled=false}};
}
async function renderDecisionCenter(){const host=qs('#decision-list');await renderDecisions();async function renderDecisions(){host.innerHTML=loading('Loading decisions…');try{const rows=await decisions(qs('#decision-filter')?.value||'all');host.innerHTML=rows.map(decisionCard).join('')||emptyState('No AI decisions','Evaluate a Lead to create a decision.');qsa('[data-approve]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await approve(b.dataset.approve,b.dataset.value==='true');toast('Decision updated','success');await renderDecisions();await refreshPending()}catch(e){toast(e.message,'error');b.disabled=false}});qsa('[data-execute]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await execute(b.dataset.execute);toast('Decision executed','success');await renderDecisions();await refreshPending()}catch(e){toast(e.message,'error');b.disabled=false}})}catch(e){host.innerHTML=emptyState('Decision log unavailable',e.message)}}}
