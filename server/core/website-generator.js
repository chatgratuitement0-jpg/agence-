...
export async function generateWebsitePreview({ projectId, userId }) {
  const db = getAdminDb();
  const { data: project, error } = await db.from('website_projects').select('*,companies(id,name,industry,city,website,email,phone),leads(id,name,recommended_service,notes)').eq('id', projectId).single();
  if (error || !project) throw new Error('Website project not found');
  if (project.leads?.id && project.leads?.owner_id && project.leads.owner_id !== userId) throw new Error('Not authorized');
  ...
  try {
    ...
    await db.from('website_projects').update({ status: 'generation_failed', notes: e.message }).eq('id', project.id);
    throw e;
  }
  ...
  const { data: updated, error: updateError } = await db.from('website_projects').update({
    ...
    notes: 'Preview generated. Final package remains gated by client approval and first payment.'
...
export async function getWebsitePreview({ projectId, token }) {
  ...
  const { data: project, error } = await db.from('website_projects').select('id,rendered_html,preview_token_hash,preview_expires_at,preview_view_count').eq('id', projectId).single();
