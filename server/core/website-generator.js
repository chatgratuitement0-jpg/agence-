...
async function getProjectForClient(db, projectId, token) {
  ...
  const { data: project, error } = await db.from('website_projects').select('id,lead_id,company_id,deal_id,status,preview_token_hash,preview_expires_at,client_approved_at,rendered_html,preview_view_count').eq('id', projectId).single();
