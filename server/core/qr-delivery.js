...
async function getQrProject(db, projectId, userId, userRole = 'sales') {
  ...
  let ownerId = null;
  if (data.lead_id) { const { data: lead } = await db.from('leads').select('owner_id').eq('id', data.lead_id).single(); ownerId = lead?.owner_id || null; }
  if (!ownerId && data.deal_id) { const { data: deal } = await db.from('deals').select('lead_id').eq('id', data.deal_id).single(); if (deal?.lead_id) { const { data: lead } = await db.from('leads').select('owner_id').eq('id', deal.lead_id).single(); ownerId = lead?.owner_id || null; } }
  if (!ownerId || ownerId !== userId) throw new Error('Not authorized');
  return data;
}

async function syncDelivery(db, project, patch = {}) {
  if (!project?.deal_id) return;
  const { data: delivery } = await db.from('deliveries').select('id,download_count').eq('deal_id', project.deal_id).eq('qr_project_id', project.id).maybeSingle();
  if (!delivery) return;
  await db.from('deliveries').update(patch).eq('id', delivery.id);
}

export async function getQrPreview({ token }) {
  const db = getAdminDb();
  const hash = crypto.createHash('sha256').update(String(token || '')).digest('hex');
  const { data, error } = await db.from('qr_projects').select('id,project_name,destination_url,rendered_svg,preview_expires_at,status').eq('preview_token_hash', hash).gt('preview_expires_at', new Date().toISOString()).single();
  if (error || !data) throw new Error('QR preview is unavailable or expired');
  return data;
}
...
export async function createQrProject({ leadId, companyId, serviceId, dealId, templateId, destinationUrl, projectName, userId, userRole = 'sales' }) {
  ...
  if (userRole !== 'admin' && userRole !== 'manager') { const { data: lead } = await db.from('leads').select('owner_id').eq('id', leadId).single(); if (!lead || lead.owner_id !== userId) throw new Error('Not authorized'); }
  const token = crypto.randomBytes(32).toString('hex');
  const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
  const { data, error } = await db.from('qr_projects').insert({ lead_id: leadId || null, company_id: companyId, service_id: serviceId || null, deal_id: dealId || null, template_id: templateId || null, project_name: projectName || 'QR Code Project', destination_url: destinationUrl, preview_token_hash: tokenHash, preview_expires_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), status: 'pending', delivery_status: 'not_ready', notes: 'QR project created. Generation and final package require approval and first payment.' }).select().single();
...
export async function generateQrPreview({ projectId, userId, userRole = 'sales' }) {
  const db = getAdminDb(); const project = await getQrProject(db, projectId, userId, userRole);
...
export async function clientReviewQr({ token, action, message = null }) {
  const db = getAdminDb(); const hash = crypto.createHash('sha256').update(String(token || '')).digest('hex');
  ...
  if (action === 'approve') {
    const { data, error: updateError } = await db.from('qr_projects').update({ client_approved_at: project.client_approved_at || new Date().toISOString(), status: 'approved_final', delivery_status: project.first_payment_at ? 'ready' : 'not_ready', notes: project.first_payment_at ? 'Client approved QR preview and first payment is verified.' : 'Client approved QR preview. Waiting for first payment before final package delivery.' }).eq('id', project.id).select().single();
    if (updateError) throw new Error(updateError.message);
    await syncDelivery(db, data, { approval_status: 'approved', payment_status: data.first_payment_at ? 'paid' : 'pending' });
    return data;
  }
...
export async function registerQrFirstPayment({ projectId, paymentId, userId, userRole = 'sales' }) {
  ...
  if (payment.deal_id && project.deal_id && payment.deal_id !== project.deal_id) throw new Error('Payment does not belong to this deal');
  if (payment.status !== 'paid') throw new Error('Payment is not marked as paid');
  const paidAt = payment.paid_at || new Date().toISOString();
  const { data, error: updateError } = await db.from('qr_projects').update({ first_payment_at: paidAt, status: 'approved_final', delivery_status: 'ready', notes: 'First payment verified. Final QR package is unlocked.' }).eq('id', projectId).select().single();
  if (updateError) throw new Error(updateError.message);
  await syncDelivery(db, data, { approval_status: 'approved', payment_status: 'paid', delivery_status: 'ready' });
  return data;
}
...
export async function generateFinalQrPackage({ projectId, userId, userRole = 'sales' }) {
  ...
  if (!project.destination_url) throw new Error('QR destination URL is required');
  if (project.package_path && project.package_generated_at) return { project, filename: project.package_path.split('/').pop(), size: project.package_size, sha256: project.package_sha256, delivery: await createDeliverySignedUrl({ path: project.package_path }), reused: true };
  ...
  const zip = makeZip([{ name: 'qr.svg', content: svg }, { name: 'destination.txt', content: `${project.destination_url}\n` }, { name: 'README.txt', content: 'QR package generated by the AI Agency CRM. Client approval and first payment were verified.\n' }]);
  const sha256 = crypto.createHash('sha256').update(zip).digest('hex'); const filename = `qr-${projectId}.zip`;
  const storage = await uploadDeliveryPackage({ projectId, zipBuffer: zip, fileName: filename }); const now = new Date().toISOString();
  const { data, error } = await db.from('qr_projects').update({ package_path: storage.path, package_size: zip.length, package_sha256: sha256, package_generated_at: now, asset_path: storage.path, status: 'packaged', delivery_status: 'ready', notes: 'Final QR package generated and uploaded to private storage.' }).eq('id', projectId).select().single();
  if (error) throw new Error(`Could not save QR package metadata: ${error.message}`);
  await syncDelivery(db, data, { approval_status: 'approved', payment_status: 'paid', production_status: 'packaged', delivery_status: 'ready' });
...
export async function downloadFinalQrPackage({ projectId, userId, userRole = 'sales' }) {
  ...
  const { data: currentDelivery } = await db.from('deliveries').select('id,download_count').eq('deal_id', project.deal_id).eq('qr_project_id', project.id).maybeSingle();
  await db.from('qr_projects').update({ download_count: Number(project.download_count || 0) + 1, delivery_status: 'delivered' }).eq('id', project.id);
  if (currentDelivery) await db.from('deliveries').update({ download_count: Number(currentDelivery.download_count || 0) + 1, delivery_status: 'downloaded', delivered_at: new Date().toISOString() }).eq('id', currentDelivery.id);
  return { projectId, filename: project.package_path.split('/').pop(), size: project.package_size, sha256: project.package_sha256, delivery };
}
