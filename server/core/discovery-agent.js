...
export async function runDiscoverySearch({ searchId, userId }) {
  ...
  await db.from('prospecting_searches').update({ status: 'running' }).eq('id', searchId).eq('created_by', userId);
  ...
  if (result.error) {
    await db.from('prospecting_searches').update({ status: 'failed', metadata: { error: result.error } }).eq('id', searchId).eq('created_by', userId);
