...
async function assertOwner(db, projectId, userId, userRole = 'sales') {
  ...
  let ownerId = null;
  if (data.lead_id) {
    const { data: lead } = await db.from('leads').select('owner_id').eq('id', data.lead_id).single(); ownerId = lead?.owner_id || null;
  } else if (data.deal_id) {
    const { data: deal } = await db.from('deals').select('lead_id').eq('id', data.deal_id).single();
    if (deal?.lead_id) { const { data: lead } = await db.from('leads').select('owner_id').eq('id', deal.lead_id).single(); ownerId = lead?.owner_id || null; }
  }
...
export async function approveWebsitePreview({ projectId, userId, userRole = 'sales' }) {
  const db = getAdminDb(); const project = await assertOwner(db, projectId, userId, userRole);
  ...
  const { data, error } = await db.from('website_projects').update({
    ...
    status: project.first_payment_at ? 'approved_final' : 'approved_final',
    delivery_status: project.first_payment_at ? 'ready' : 'not_ready',
    delivery_notes: project.first_payment_at ? 'Client approved preview and first payment is verified.' : 'Client approved preview. Waiting for first payment.'
...
export async function registerFirstPayment({ projectId, paymentId, userId, userRole = 'sales' }) {
  ...
  if (payment.milestone_type && !['deposit','remaining_balance','full_payment'].includes(payment.milestone_type)) throw new Error('Invalid payment milestone type');
  const paidAt = payment.paid_at || new Date().toISOString();
  const { data, error } = await db.from('website_projects').update({ first_payment_at: paidAt, status: 'approved_final', delivery_status: 'ready', delivery_notes: 'First payment verified. Final package is now unlocked.' }).eq('id', projectId).select().single();
  if (error) throw new Error(error.message); return data;
}
...
export async function generateFinalWebsitePackage({ projectId, userId, userRole = 'sales' }) {
  ...
  if (project.package_path && project.package_generated_at) {
    return { project, filename: project.package_path.split('/').pop(), sha256: project.final_zip_sha256, size: project.final_zip_size, delivery: await createDeliverySignedUrl({ path: project.package_path }), reused: true };
  }
  const zip = makeZip([{ name: 'index.html', content: project.rendered_html }, { name: 'README.txt', content: 'Website package generated by the AI Agency CRM. The client preview was approved and the first payment was verified.' }]);
  const sha256 = crypto.createHash('sha256').update(zip).digest('hex'); const filename = `website-${projectId}.zip`;
  const storage = await uploadDeliveryPackage({ projectId, zipBuffer: zip, fileName: filename }); const now = new Date().toISOString();
  const { data, error } = await db.from('website_projects').update({ package_generated_at: now, final_zip_generated_at: now, final_zip_size: zip.length, final_zip_sha256: sha256, package_path: storage.path, delivery_status: 'ready', status: 'packaged', delivery_notes: 'Final package generated and uploaded to private storage.' }).eq('id', projectId).select().single();
...
export async function downloadFinalWebsitePackage({ projectId, userId, userRole = 'sales' }) {
  ...
  return { projectId, filename: project.package_path.split('/').pop(), sha256: project.final_zip_sha256, size: project.final_zip_size, delivery };
}
